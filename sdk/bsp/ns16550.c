...
#if defined(CONFIG_SERIAL_PL011)
...
#elif defined(CONFIG_SERIAL_PIO_BASE)
static struct console_uart uart = {
	.enabled = true,
	.type = PIO,
 ...
 */
static inline uint32_t serial_read_reg(struct console_uart uart, uint16_t reg_idx)
{
	if (uart.type == PIO) {
 ...
 * @pre uart->enabled == true
 */
static inline void serial_write_reg(struct console_uart uart, uint32_t val, uint16_t reg_idx)
{
	if (uart.type == PIO) {
...
static void serial_calc_baud_div(uint32_t ref_freq, uint32_t *baud_div_ptr, uint32_t baud_rate_arg)
{
 ...
 * @pre uart->enabled == true
 */
static void serial_set_baud_rate(uint32_t baud_rate)
{
	uint32_t baud_div, duart_clock = UART_CLOCK_RATE;
	uart_reg_t temp_reg;

	/* Calculate baud divisor */
	serial_calc_baud_div(duart_clock, &baud_div, baud_rate);

	/* Enable DLL and DLM registers for setting the Divisor */
	temp_reg = serial_read_reg(uart, UART16550_LCR);
	temp_reg |= LCR_DLAB;
	serial_write_reg(uart, temp_reg, UART16550_LCR);

	/* Write the appropriate divisor value */
	serial_write_reg(uart, ((baud_div >> 8U) & 0xFFU), UART16550_DLM);
	serial_write_reg(uart, (baud_div & 0xFFU), UART16550_DLL);
...
void serial_init(bool early_boot)
{
	...
	serial_write_reg(uart, FCR_FIFOE | FCR_RFR | FCR_TFR, UART16550_FCR);

	/* Set-up data bits / parity / stop bits. */
	serial_write_reg(uart, (LCR_WL8 | LCR_NB_STOP_BITS_1 | LCR_PARITY_NONE), UART16550_LCR);

	/* Disable interrupts (we use polling) */
	serial_write_reg(uart, UART_IER_DISABLE_ALL, UART16550_IER);

	/* Set baud rate */
	serial_set_baud_rate(BAUD_115200);

	/* Data terminal ready + Request to send */
	serial_write_reg(uart, MCR_RTS | MCR_DTR, UART16550_MCR);
}
...
char serial_getc(void)
{
	...
	if ((serial_read_reg(uart, UART16550_LSR) & LSR_DR) == LSR_DR) {
		ret = (char)serial_read_reg(uart, UART16550_RBR);
	}
...
static void serial_putc(char c)
{
	...
	do {
		reg = serial_read_reg(uart, UART16550_LSR);
	} while ((reg & LSR_THRE) == 0U || (reg & LSR_TEMT) == 0U);

	temp = (uint8_t)c;
	/* Transmit the character. */
	serial_write_reg(uart, (uint32_t)temp, UART16550_THR);
}
