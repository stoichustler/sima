...
#ifndef CONFIG_HAS_HSM
#define CONFIG_HAS_HSM	1
#endif
...
__unused static void acrn_print_request(uint16_t vcpu_id, const struct acrn_io_request *req)
{
	...
	case ACRN_IOREQ_TYPE_MMIO:
		dev_dbg(DBG_LEVEL_IOREQ, "[vcpu%hu type=mmio]", vcpu_id);
	...
	case ACRN_IOREQ_TYPE_PORTIO:
		dev_dbg(DBG_LEVEL_IOREQ, "[vcpu%hu type=portio]", vcpu_id);
		dev_dbg(DBG_LEVEL_IOREQ,
		...
		break;
	default:
		dev_dbg(DBG_LEVEL_IOREQ, "[vcpu%hu type=%d] not support type",
			vcpu_id, req->type);
		break;
	}
}
...
#else
int add_asyncio(__unused struct acrn_vm *vm,
	__unused const struct acrn_asyncio_info *async_info)
{
	return -ENODEV;
}

int remove_asyncio(__unused struct acrn_vm *vm,
	__unused const struct acrn_asyncio_info *async_info)
{
	return -ENODEV;
}

int32_t acrn_insert_request(__unused struct acrn_vcpu *vcpu,
	__unused const struct io_request *io_req)
{
	return -ENODEV;
}

uint32_t get_io_req_state(__unused struct acrn_vm *vm, __unused uint16_t vcpu_id)
{
	return ACRN_IOREQ_STATE_FREE;
}

void set_io_req_state(__unused struct acrn_vm *vm, __unused uint16_t vcpu_id,
	__unused uint32_t state)
{
}

int init_asyncio(__unused struct acrn_vm *vm, __unused uint64_t *hva)
{
	return -ENODEV;
}

void set_hsm_notification_vector(__unused uint32_t vector)
{
}
